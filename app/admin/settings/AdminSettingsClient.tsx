...
import { Announcement } from '@/store/announcements.store'
...
export default function AdminSettingsClient({ initialSettings, initialAnnouncements }: AdminSettingsClientProps) {
  ...
  const handleToggleAnnouncement = async (id: string, currentStatus: boolean) => {
    try {
      await apiToggleAnnouncementActive(id, !currentStatus)
      router.refresh()
  ...
  const handleUpdateAnnouncementText = async (id: string, updates: Partial<Announcement>) => {
     try {
       await apiUpdateAnnouncement(id, updates)
       router.refresh()
     } catch (err: any) {
       toast.error('Erreur: ' + err.message)
     }
                  ...
                  <section className="space-y-6">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      ...
                      <div className="p-6 bg-amber-50 rounded-2xl border border-amber-100 space-y-4">
                         <label className="text-[8px] font-black uppercase text-amber-950/30 block">Minimum de Commande (DZD)</label>
                         <input type="number" name="minOrderAmount" value={formData.minOrderAmount} onChange={handleChange} className="w-full h-12 px-4 bg-white border border-amber-100 rounded-xl font-bold text-emerald-950 outline-none" />
                      </div>
                    </div>
                  </section>
              ...
              {activeTab === 'Annonces' && (
                <section className="space-y-8">
                  <div className="flex items-center justify-between border-b border-emerald-950/5 pb-4">
                    ...
                    <span className="text-[9px] font-black text-emerald-950/30 uppercase tracking-widest">{initialAnnouncements.length} Annonce(s)</span>
                  </div>
                  ...
                  <div className="space-y-3">
                    {initialAnnouncements.length === 0 ? (
                      ...
                      initialAnnouncements.sort((a,b) => a.display_order - b.display_order).map((ann, idx) => (
                        <div key={ann.id} className="group flex flex-col md:flex-row items-center gap-4 p-4 bg-white border border-emerald-950/5 rounded-2xl hover:border-emerald-950/20 transition-all">
                           ...
                           <div className="flex-1 grid grid-cols-1 md:grid-cols-2 gap-4 w-full">
                              <input 
                                ...
                                value={ann.text_fr}
                                onChange={e => handleUpdateAnnouncementText(ann.id, { text_fr: e.target.value })}
                              ...
                              <input 
                                ...
                                value={ann.text_ar}
                                onChange={e => handleUpdateAnnouncementText(ann.id, { text_ar: e.target.value })}
                                placeholder="النص العربي..."
                                dir="rtl"
                              />
                           </div>
