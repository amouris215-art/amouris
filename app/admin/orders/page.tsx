...
export default function AdminOrdersPage() {
  ...
  const filtered = orders.filter(o => {
    const customer = o.customer_id ? customers.find(c => c.id === o.customer_id) : null
    const name = customer ? `${customer.first_name} ${customer.last_name}` : `${o.guest_first_name} ${o.guest_last_name}`
    const matchSearch = o.order_number.toLowerCase().includes(search.toLowerCase()) || name.toLowerCase().includes(search.toLowerCase())
      ...
      <div className="bg-white rounded-[2.5rem] border border-emerald-50 overflow-hidden shadow-sm">
        <div className="overflow-x-auto">
            ...
            <tbody className="divide-y divide-emerald-50 text-sm">
                ...
                const name = customer ? `${customer.first_name} ${customer.last_name}` : `${order.guest_first_name} ${order.guest_last_name}`
                return (
                    <motion.tr 
                        layout
                        initial={{ opacity: 0 }}
                    ...
                    <td className="px-8 py-6">
                        <div className="font-mono font-black text-emerald-900 text-base">{order.order_number}</div>
                        <div className="text-[10px] text-emerald-950/30 font-bold mt-1 uppercase tracking-widest">{new Date(order.created_at).toLocaleDateString()}</div>
                    </td>
                    <td className="px-8 py-6">
                        <div className="font-bold text-emerald-950">{name}</div>
                        {!customer && <div className="text-[10px] text-amber-600 font-black uppercase tracking-tighter">Client invité</div>}
                        {customer?.shop_name && <div className="text-xs text-emerald-900/40">{customer.shop_name}</div>}
                    </td>
                    <td className="px-8 py-6">
                        <div className="font-sans font-black text-emerald-950 text-lg">{order.total_amount.toLocaleString()} <span className="text-[10px] font-normal">DZD</span></div>
                    </td>
                    <td className="px-8 py-6">
                    ...
                    <td className="px-8 py-6">
                        <span className={`text-[10px] px-3 py-1.5 rounded-full font-black uppercase tracking-widest ${
                            ...
                            {order.payment_status === 'paid' ? 'Payé' : order.payment_status === 'partial' ? 'Partiel' : 'À régler'}
                        </span>
                    </td>
                    <td className="px-8 py-6">
