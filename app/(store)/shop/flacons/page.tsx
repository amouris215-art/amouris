...
export default function FlaconsPage() {
  ...
  useEffect(() => {
    async function loadProducts() {
      try {
        const data = await getProducts({ type: 'flacon', status: 'active' })
        setProducts(data)
      } catch (error) {
        console.error('Failed to fetch flacons:', error)
      } finally {
        setIsLoading(false)
      }
    }
      ...
      <div className="bg-emerald-950 py-24 text-center relative overflow-hidden">
        ...
        >
            ...
            <p className="text-emerald-200/60 text-sm md:text-base uppercase tracking-widest font-medium">
                {isLoading ? '...' : products.length} {language === 'ar' ? 'نموذج متاح' : 'Modèles exclusifs'} — {language === 'ar' ? 'جودة بريميوم' : 'Qualité Premium'}
            </p>
        </motion.div>
      </div>
